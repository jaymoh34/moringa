...
def my_deco(func):
    def wrapper():
        print("Before we call the function")  # change this bit
        func()  # try when comments are on and off
        print("After we call the function")  # change this bit
    return wrapper
...
# → french_hello → my_deco(french_hello) → wrapper() → french_hello() → hello
french_hello()